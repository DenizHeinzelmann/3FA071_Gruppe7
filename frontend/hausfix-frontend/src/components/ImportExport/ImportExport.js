      ...
      try {
        ...
        switch (fileFormat) {
          ...
          case 'xml':
            ...
            data = dataType === 'customers' ? result.customers.customer : result.readings.reading;
            break;
          case 'csv':
            const parsed = Papa.parse(event.target.result, { header: true });
            data = parsed.data;
            break;
          default:
            return;
      ...
      <Grid container spacing={3}>
        ...
        <Grid item xs={12} sm={6}>
          <Button variant="contained" component="label" color="secondary" fullWidth>
            Importieren
            <input type="file" hidden onChange={handleImport} accept={fileFormat === 'json' ? '.json' : fileFormat === 'xml' ? '.xml' : '.csv'} />
          </Button>
        </Grid>
      </Grid>
    </Paper>
